...
export default function CatalogGrid({ products }: { products: Product[] }) {
  ...
  return (
    <>
      {/* ═══ Filters & Sort ═══ */}
      <section className="sticky top-0 z-30 bg-white/90 backdrop-blur-md border-b border-cream/30 shadow-sm">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {ROAST_OPTIONS.map((option) => (
                <button
                  ...
                  className={`px-4 py-1.5 rounded-full font-heading text-xs tracking-[0.12em] uppercase transition-all duration-300 cursor-pointer ${
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <AnimatedReveal>
            <p className="font-body text-sm text-espresso/50 mb-8">
              ...
              {productCount === 1 ? 'товар' : productCount >= 2 && productCount <= 4 ? 'товара' : 'товаров'}
            </p>
          </AnimatedReveal>
            ...
            <AnimatedReveal>
              <div className="flex flex-col items-center justify-center py-24 text-center">
                <div className="w-20 h-20 rounded-full bg-cream/30 flex items-center justify-center mb-6">
                  <svg className="w-10 h-10 text-espresso/30" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
                ...
                <p className="font-body text-sm text-espresso/40 max-w-sm mb-8">
                  По выбранным фильтрам нет подходящих товаров. Попробуйте изменить параметры.
                </p>
                <button
                  onClick={() => { setRoastFilter('все'); setSortOption('default'); }}
